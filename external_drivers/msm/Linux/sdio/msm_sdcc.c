...
// #define TRIGGLE_GPIO_WHEN_ERROR

#ifdef TRIGGLE_GPIO_WHEN_ERROR
extern int gpio_direction_output(unsigned gpio, int value);

#define TRIGGER_GPIO_NUM 96
#define LOW     0
#define HIGH    1
#endif
...
msmsdcc_pio_irq(int irq, void *dev_id)
{
	...
	if (host->pio.sg == NULL) {
		...
		if (status & MCI_RXACTIVE) {
			int read_cnt = 0;
			while (readl(base+MMCISTATUS) & MCI_RXDATAAVLBL) {
				readl(base + MMCIFIFO + (read_cnt % MCI_FIFOSIZE));
				read_cnt += sizeof(uint32_t);
				if ((read_cnt) > MCI_FIFOSIZE)
					break;
			}
			writel(MCI_RXDATAAVLBLMASK, base+MMCIMASK1);
			printk("msmsdcc_pio_irq RX\n");
		}
...
msmsdcc_irq(int irq, void *dev_id)
{
	...
	do {
		...
		if (status & (MCI_DATACRCFAIL | MCI_DATATIMEOUT)) {
			...
			gpio_direction_output(TRIGGER_GPIO_NUM,HIGH);
            udelay(1);
            gpio_direction_output(TRIGGER_GPIO_NUM,LOW);

			printk(KERN_INFO "------ GPIO%d triggerred ------\n", TRIGGER_GPIO_NUM);
			data = host->curr.data;

#if 0
			if (data) {
				buffer = kmap_atomic(sg_page(data->sg), KM_BIO_SRC_IRQ);
				buffer_len = data->blksz * data->blocks;
				printk("\n********************** CurrentData : blksz %d, blocks 0x%.8x, error 0x%.8x, flags 0x%.8x **********************\n",
				data->blksz, data->blocks, data->error, data->flags);
...
msmsdcc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (ios->clock > 400000 && msmsdcc_pwrsave) {
		clk |= (1 << 9); /* PWRSAVE */
		printk("[sdio] msm_sdcc pwr saving mode ENABLED\n");
	} else {
		printk("[sdio] msm_sdcc pwr saving mode DISABLE\n");
	}
	clk |= (1 << 12); /* FLOW_ENA */
...
int msmsdcc_probe(struct platform_device *pdev)
{
	...
	ret = request_irq(pio_irqres->start, (irq_handler_t) msmsdcc_pio_irq, IRQF_SHARED,
