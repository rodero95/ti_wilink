...
#define CONFIG_MUX_MODE_0                  (0)
#define CONFIG_MUX_PULL_UP_ENABLED         (3)
...
static int OMAP2430_pad_config(unsigned int reg_offset, unsigned char mux_mode, unsigned char pull)
{
	...
	if (reg_offset) 
	{
		...
		writeb(reg,TESTDRV_OMAP24XX_VA_SYSTEM_CONTROL_BASE + reg_offset);
	}
	else
	{
...
{
  printk("OMAP2430_TNETW_Power(-%s-)\n", power_on ? "ON" : "OFF");

  omap_set_gpio_direction(TESTDRV_GPIO_10, TESTDRV_GPIO_OUTPUT);

  if (power_on)
  {
    omap_set_gpio_dataout(TESTDRV_GPIO_10, 1);
  }
  else
  {
    omap_set_gpio_dataout(TESTDRV_GPIO_10, 0);
  }
...
static int OMAP2430_TNETW_HardReset(void)
{
  int err;
...
} /* OMAP2430_TNETW_HardReset() */


/*--------------------------------------------------------------------------------------*/

static int OMAP2430_Wlan_Hardware_Init(void)
{
  int err;

  printk("OMAP2430_Wlan_Hardware_Init() enter\n");
  do
  {
	err = OMAP2430_pad_config(TESTDRV_CONTROL_PADCONF_UART1_TX_OFFSET, TESTDRV_MUXMODE_3, 0);
	if (err)
	{
	  printk ("OMAP2430_Wlan_Hardware_Init: OMAP2430_pad_config(TESTDRV_CONTROL_PADCONF_UART1_TX_OFFSET) FAILD\n");
	  break;
	}
	err = OMAP2430_pad_config(TESTDRV_CONTROL_PADCONF_UART1_RX_OFFSET, TESTDRV_MUXMODE_3, 3);
	if (err)
	{
	  printk ("OMAP2430_Wlan_Hardware_Init: OMAP2430_pad_config(TESTDRV_CONTROL_PADCONF_UART1_RX_OFFSET) FAILD\n");
	  break;
	}
	/* 
	 * set pull up on all SDIO lines from the 2430 registers 
	 */
	err = OMAP2430_pad_config (CONTROL_MMC2_CMD_PAD_PA, CONFIG_MUX_MODE_0, 3);
	if (err)
	{
      printk ("OMAP2430_Wlan_Hardware_Init: ERROR setting CONTROL_MMC2_CMD_PAD_PA\n");
	  break;
	}
	err = OMAP2430_pad_config (CONTROL_MMC2_DAT3_PAD_PA, CONFIG_MUX_MODE_0, 3);
	if (err)
	{
      printk ("OMAP2430_Wlan_Hardware_Init: ERROR setting CONTROL_MMC2_DAT3_PAD_PA\n");
	  break;
	}
	err = OMAP2430_pad_config (CONTROL_MMC2_DAT2_PAD_PA, CONFIG_MUX_MODE_0, 3);
	if (err)
	{
      printk ("OMAP2430_Wlan_Hardware_Init: ERROR setting CONTROL_MMC2_DAT2_PAD_PA\n");
	  break;
	}
	err = OMAP2430_pad_config (CONTROL_MMC2_DAT1_PAD_PA, CONFIG_MUX_MODE_0, 3);
	if (err)
	{
      printk ("OMAP2430_Wlan_Hardware_Init: ERROR setting CONTROL_MMC2_DAT1_PAD_PA\n");
	  break;
	}
	err = OMAP2430_pad_config (CONTROL_MMC2_DAT0_PAD_PA, CONFIG_MUX_MODE_0, 3);
	if (err)
	{
      printk ("OMAP2430_Wlan_Hardware_Init: ERROR setting CONTROL_MMC2_DAT0_PAD_PA\n");
	}
  } while(0);
  printk("OMAP2430_Wlan_Hardware_Init() exit %d\n", err);

  return err;

} /* OMAP2430_Wlan_Hardware_Init */
...
static void sdio_test_reset(void)
{
	...
	printk("OMAP2430_TNETW_HardReset start\n");
	err = OMAP2430_TNETW_HardReset(); 
	printk("OMAP2430_TNETW_HardReset - %s\n", err ? "FAIL" : "SUCCESS");
...
static void usage(void)
{
  ...
  printk("    sf                      - sync function-0 write-read-compare test\n");
  printk("    sa                      - sync function-0 read test\n\n");
    ...
    case 'Z':
        {
            ...
            /* write to 2430 SYSCTL register */
            __raw_writel(divider, IO_ADDRESS(OMAP_HSMMC2_BASE) + 0x012C);
        }
        break;
    case 'f':
    case 'F':
...
static int  __init sdio_test_module_init(void)
{
	...
	if (omap_request_gpio(TESTDRV_GPIO_10) != 0) 
	{
	  printk(KERN_ERR "%s:OMAP2430_TNETW_Power() omap_request_gpio FAILED\n",__FILE__);
	  return -EINVAL;
	}
	printk("OMAP2430_Wlan_Hardware_Init start\n");
	err = OMAP2430_Wlan_Hardware_Init(); 
	printk("OMAP2430_Wlan_Hardware_Init - %s\n", err ? "FAIL" : "SUCCESS");
	...
	if (omap_request_gpio(TESTDRV_GPIO_9) != 0) 
    {
	    printk(KERN_ERR "sdio_test_module_init() omap_request_gpio() FAILED !!\n");
		return -EINVAL;
	};
	omap_set_gpio_direction(TESTDRV_GPIO_9, OMAP24XX_DIR_INPUT);
	/*	set_irq_type (TESTDRV_TIWLAN_OMAP2430_IRQ, OMAP_GPIO_FALLING_EDGE); */
	omap_set_gpio_edge_ctrl(TESTDRV_GPIO_9, OMAP_GPIO_FALLING_EDGE);
	if (request_irq (TESTDRV_TIWLAN_IRQ, tiwlan_interrupt, SA_SHIRQ, TESTDRV_MODULE_NAME, &g_sdio))
	{
...
static void __exit sdio_test_module_exit(void)
{
	...
	omap_free_gpio(TESTDRV_GPIO_9);
	omap_free_gpio(TESTDRV_GPIO_10);
...
MODULE_DESCRIPTION("OMAP2430 MMC Test");
MODULE_LICENSE("GPL");
